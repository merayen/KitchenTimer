 ...
 * @property runningTimerName Name of the owner
...
data class RunningTimer(
    ...
    val progress: Float
        get() = if (start > 0)
            max(0f, min(1f, (elapsed + System.currentTimeMillis() / 1000 - start) / seconds.toFloat()))
        else
            max(0f, min(1f, elapsed / seconds.toFloat()))
}